...
const DATA: CityData[] = [
  {
    city: "Красноярск",
    months: {
      "янв": { plan: 33000000, fact: 30234890 },
      "фев": { plan: 38000000, fact: 43446100 },
      "мар": { plan: 40000000, fact: 0 },
    },
  },
  {
    city: "Санкт-Петербург",
    months: {
      "янв": { plan: 40000000, fact: 35080383.23 },
      "фев": { plan: 40000000, fact: 50000017.88 },
      "мар": { plan: 43000000, fact: 0 },
    },
  },
  {
    city: "Омск",
    months: {
      "янв": { plan: 35000000, fact: 30180644.86 },
      "фев": { plan: 35000000, fact: 35326751.87 },
      "мар": { plan: 40000000, fact: 0 },
    },
  },
  {
    city: "Кемерово",
    months: {
      "янв": { plan: 35000000, fact: 25334148.26 },
      "фев": { plan: 40000000, fact: 30779228.15 },
      "мар": { plan: 35000000, fact: 0 },
    },
  },
  {
    city: "Краснодар Буд",
    months: {
      "янв": { plan: 20000000, fact: 10505380 },
      "фев": { plan: 18000000, fact: 6452875 },
      "мар": { plan: 20000000, fact: 0 },
    },
  },
  {
    city: "Краснодар Сев",
    months: {
      "янв": { plan: 10000000, fact: 4557125 },
      "фев": { plan: 10000000, fact: 8927550 },
      "мар": { plan: 15000000, fact: 0 },
    },
  },
  {
    city: "Самара",
    months: {
      "янв": { plan: 55000000, fact: 50250950 },
      "фев": { plan: 60000000, fact: 50275700 },
      "мар": { plan: 60000000, fact: 0 },
    },
  },
  {
    city: "Пермь",
    months: {
      "янв": { plan: 40000000, fact: 29290277.16 },
      "фев": { plan: 40000000, fact: 41177550.50 },
      "мар": { plan: 45000000, fact: 0 },
    },
  },
  {
    city: "Ростов",
    months: {
      "янв": { plan: 38000000, fact: 34178070 },
      "фев": { plan: 50000000, fact: 42808575 },
      "мар": { plan: 43000000, fact: 0 },
    },
  },
  {
    ...
    months: {
      "янв": { plan: 40000000, fact: 34140224 },
      "фев": { plan: 40000000, fact: 36102343 },
      "мар": { plan: 42000000, fact: 0 },
    },
  },
  {
    city: "Новосибирск",
    months: {
      "янв": { plan: 35000000, fact: 22840437 },
      "фев": { plan: 35000000, fact: 26769030 },
      "мар": { plan: 35000000, fact: 0 },
    },
  },
  {
    city: "Калининград",
    months: {
      "янв": { plan: 35000000, fact: 23451002.07 },
      "фев": { plan: 35000000, fact: 19217643.66 },
      "мар": { plan: 35000000, fact: 0 },
    },
  },
  {
    city: "Барнаул",
    months: {
      "янв": { plan: 22000000, fact: 10673705 },
      "фев": { plan: 15000000, fact: 11572085 },
      "мар": { plan: 17000000, fact: 0 },
    },
  },
  {
    city: "Тольятти",
    months: {
      "янв": { plan: 15000000, fact: 10851500 },
      "фев": { plan: 15000000, fact: 9345600 },
      "мар": { plan: 15000000, fact: 0 },
    },
  },
  {
    city: "Улан-Удэ",
    months: {
      "янв": { plan: 25000000, fact: 14571465 },
      "фев": { plan: 25000000, fact: 20542472.90 },
      "мар": { plan: 30000000, fact: 0 },
    },
  },
  {
    city: "Новокузнецк",
    months: {
      "янв": { plan: 40000000, fact: 28526884.01 },
      "фев": { plan: 40000000, fact: 29694730.69 },
      "мар": { plan: 38000000, fact: 0 },
    },
  },
];
...
export default function VyrabotkaView() {
  ...
  const getCityTotals = (city: CityData) => {
    let plan = 0, fact = 0;
    const ms = selectedMonth ? [selectedMonth] : activeMonths;
    ms.forEach(m => {
  ...
  const bestCity = cityRanking.find(c => c.plan > 0);
  const worstCity = [...cityRanking].reverse().find(c => c.plan > 0);

  const barData = (selectedCity ? DATA.filter(d => d.city === selectedCity) : DATA).map(d => {
    const t = getCityTotals(d);
    return { name: d.city, plan: t.plan, fact: t.fact };
  }).sort((a, b) => b.plan - a.plan);

  const monthlyData = activeMonths.map(m => {
    let plan = 0, fact = 0;
    (selectedCity ? DATA.filter(d => d.city === selectedCity) : DATA).forEach(d => {
      const md = d.months[m];
      if (md) { plan += md.plan; fact += md.fact; }
    });
    return { name: MONTH_LABELS[m] || m, plan, fact, pct: plan > 0 ? ((fact / plan) * 100) : 0 };
  });
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="glass rounded-2xl p-6 animate-fade-in-up">
          <div className="mb-4">
            <h3 className="font-display font-bold text-white text-lg">Топ городов</h3>
            <p className="text-white/40 text-xs mt-0.5">По % выполнения плана</p>
          </div>
          <div className="space-y-3 max-h-[280px] overflow-y-auto pr-1">
            {cityRanking.filter(c => c.plan > 0).map((c, i) => (
              <div key={c.city}
                className="flex items-center gap-3 cursor-pointer hover:bg-white/5 rounded-xl p-2 transition-colors"
                onClick={() => setSelectedCity(selectedCity === c.city ? null : c.city)}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                  i === 0 ? "bg-emerald-500/20 text-emerald-400" :
                  i === cityRanking.filter(c2 => c2.plan > 0).length - 1 ? "bg-red-500/20 text-red-400" :
                ...
                <span className={`text-sm font-bold ${pctColor(c.pct)}`}>
                  {c.pct.toFixed(1)}%
                </span>
              </div>
            ))}
        ...
        <div className="glass rounded-2xl p-6 animate-fade-in-up">
          ...
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={barData} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="plan" name="План" fill="#7C5CFF" radius={[0, 4, 4, 0]} barSize={14} />
              <Bar dataKey="fact" name="Факт" fill="#00E5CC" radius={[0, 4, 4, 0]} barSize={14} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
              ...
              {DATA.map(d => {
                ...
                return (
                  <tr key={d.city}
                    ...
                    <td className="py-3 px-3 text-right text-white/70">{fmtFull(t.plan)}</td>
                    <td className="py-3 px-3 text-right text-white">{t.fact > 0 ? fmtFull(t.fact) : "—"}</td>
                    <td className={`py-3 px-3 text-right font-semibold ${t.diff >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                      {t.fact > 0 ? ((t.diff >= 0 ? "+" : "") + fmtFull(t.diff)) : "—"}
                    </td>
                    <td className="py-3 px-3 text-right">
                      {t.fact > 0 ? (
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${pctBg(t.pct)}`}>
                          {t.pct.toFixed(1)}%
                        </span>
                      ) : "—"}
                    </td>
