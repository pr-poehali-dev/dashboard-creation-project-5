            ...
            <div className="flex flex-col justify-center gap-3">
              {(() => {
                ...
                return cityProfileData
                  ...
                  .map((d, i) => {
                    const cityW = Math.max((d.cityRaw / maxVal) * 100, 2);
                    const avgW = Math.max((d.avgRaw / maxVal) * 100, 2);
                    const diff = d.cityRaw - d.avgRaw;
                    const pct = d.avgRaw > 0 ? Math.round((diff / d.avgRaw) * 100) : 0;
                    return (
                      <div key={i}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm text-white/60 truncate max-w-[55%]" title={d.fullLabel}>{d.fullLabel}</span>
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-bold" style={{ color: d.color }}>{d.cityRaw.toLocaleString("ru-RU")}</span>
                            <span className="text-xs text-white/30">vs {d.avgRaw.toLocaleString("ru-RU")}</span>
                            {diff !== 0 && (
                        ...
                        <div className="relative h-3.5 rounded-full overflow-hidden" style={{ background: isLight ? "rgba(20,10,40,0.06)" : "rgba(255,255,255,0.06)" }}>
                          <div className="absolute inset-y-0 left-0 rounded-full transition-all duration-500"
                            style={{ width: `${cityW}%`, background: d.color, opacity: 0.85 }} />
                          <div className="absolute top-0 h-full w-1 rounded-full transition-all duration-500"
                            style={{ left: `${avgW}%`, background: "#00BFFF", opacity: 0.7 }} />
                        </div>
                      </div>
                    );
                  });
              })()}
              <div className="flex items-center gap-5 mt-2 pt-2 border-t border-white/8">
                <span className="flex items-center gap-2 text-xs text-white/40">
                  <span className="w-5 h-2 rounded-full bg-violet-500 opacity-80" /> {selectedCity}
                </span>
                <span className="flex items-center gap-2 text-xs text-white/40">
                  <span className="w-1 h-3 rounded-full bg-cyan-400 opacity-60" /> Среднее
                </span>
              </div>
            </div>
          </div>
